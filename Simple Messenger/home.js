...
var receiver; //window.prompt("Who do you want to text: ");
...
var j=0, temp=0,last=null;
get_friends();
function gotData(data) {
  var text = data.val();
  var s = "", i=0;
  var keys  = Object.keys(text);
  window.temp = keys.length-1;
  for(i = j ; i < keys.length ; i++)
  {
    var k = keys[i];
    ...
    txt.innerHTML = text[k].data;
    if(text[k].user==sender)
    {
        name.className="shit yous";
        txt.className="ALL TXT you";
	}
    else
    {
        txt.className="ALL TXT other";
        name.className="shit others";
	}
...
function go(){
  var dat=document.getElementById("inpval").value;
  var b=0;
...
function del(user_name)
{
    ...
    if(sender < receiver)
        hash = md5(sender+receiver);
    else hash = md5(receiver+sender);
    j=0, temp=0;
    ref = database.ref("data/" + hash);
    ref.on("value", gotData);
    

}
function add_user(user_name="nazmul2")
{
     var box = document.createElement("div");
     box.className="chat_users";
     box.id=user_name;
     box.innerHTML=user_name;
     box.setAttribute( "onClick", "del(id)" );
     document.getElementById("user_list").appendChild(box);
}
function get_friends()
{
    database.ref("friends/" + md5(sender)).on("value",get);
...
function get(data)
{
  ...
  var val = {
    ...
    database.ref("friends/" + md5(sender)).push(val);
    return;
  }
  var i=0;
  var keys  = Object.keys(text);
  for(i = j2 ; i < keys.length ; i++)
  {
    var k = keys[i];
    if(text[k].username!=sender)
    add_user(text[k].username);
  }
  j2=i;
}
...
function addusr()
{
  var dat=document.getElementById("adduser").value;
  var b=1;
  for(var i=0;i<dat.length;i++)
  if(dat[i]==' ')
    {
  ...
  if(dat != "" && b==1)
  {
    ...
    schr=dat;
    database.ref("friends/" + md5(sender)).on("value",search);
    if(f==1){
      database.ref("friends/" + md5(sender)).push(val);
      database.ref("friends/" + md5(sender)).push(val);
    }
